/**
 * src/Components/MyPolls/MyPolls.jsx
 * ---------------------------------------------------------------------------
 * Changes from the original:
 *
 * 1. Both network calls (`/v1/myPolls` for the poll list, and
 *    `/v1/results/:pollId` inside the PDF generator) now go through
 *    `apiFetch`. That means: the httpOnly session cookie is sent
 *    automatically, so we no longer need to resend `Student_ID` /
 *    `password` in the body — `protect` on the backend identifies the
 *    caller from the verified JWT. Body is now empty for both calls.
 *
 * 2. The PDF export previously hardcoded "Aga Khan High School, Kampala"
 *    and a fixed `/Aga-Khan.png` logo — meaning every school's exported
 *    results PDF would say Aga Khan regardless of which tenant generated
 *    it. It now takes `schoolName` / `logoUrl` as props (sourced from the
 *    tenant config the parent already has available via
 *    `GET /v1/school-config`, the same endpoint `CreatePoll.jsx` uses)
 *    and falls back to VoteAble's own generic branding if none is
 *    supplied, rather than silently mislabeling another school's
 *    election results as Aga Khan's.
 */
...
const ElectionResultsPDF = ({ polls = [], onClose, schoolName, logoUrl }) => {
  ...
  @media print { body { background-color: white !important; } .poll-section { box-shadow: none; } }
          ...
          <body>
            ...
            <div class="footer">
              ...
              <p>Generated by VoteAble Election System | ${displayName}</p>
            </div>
          </body>
        </html>
      `;
...
function MyPolls() {
  ...
  useEffect(() => {
    // Fetch tenant branding once, used only for the PDF export header.
    apiFetch('/v1/school-config')
      .then((res) => res.json())
      .then((data) => {
        if (data?.data) setSchoolConfig(data.data);
      })
      .catch((err) => console.error('Could not load school config:', err));
  }, []);
  ...
  useEffect(() => {
    const myPolls = async () => {
      ...
      try {
        // No more resending Student_ID/password — `apiFetch` carries the
        // session cookie, and the backend identifies the caller (and
        // whether they're an admin) from that verified session.
    ...
    <div className="mypolls-page">
      ...
      {signupFirstErr && (
        <div className="pollc">
          ...
          <p style={{ marginLeft: '10px', marginRight: '10px' }}>
            Please login with valid credentials to vote as a student of{' '}
            {schoolConfig?.name || 'your school'}. <br /> <br />
      ...
      {dataLoaded && showPDFModal && (
        <ElectionResultsPDF
          ...
          logoUrl={schoolConfig?.branding?.logoUrl}
        />
      )}
    </div>
  );
